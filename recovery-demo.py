...
# ----------------------------------------------------------------
# Helper: safe peak-to-peak (works in all NumPy versions)
# ----------------------------------------------------------------

def safe_ptp(arr):
    """Compute peak-to-peak (max - min) compatible with NumPy 2.0+."""
    arr = np.asarray(arr)
...
def plot_voronoi_from_matrix(C, block_sizes=None, generators=None,
    ...
    # ---- FIX: use safe_ptp instead of .ptp() ----
    mx = 0.15 * (safe_ptp(all_pts[:, 0]) or 1)
    my = 0.15 * (safe_ptp(all_pts[:, 1]) or 1)
...
if __name__ == "__main__":
    ...
    # ============================================================
    # Demo 2  –  Hand-crafted cells
    # ============================================================
    print("\n" + "=" * 70)
    print(" DEMO 2: Manual Cells → Tessellation Matrix → Recovery")
    print("=" * 70)

    cell_A = np.array([0+0j, 2+0j, 2+2j, 0+2j])        # square
    cell_B = np.array([2+0j, 4+0j, 4+2j, 2+2j])         # square
    cell_C = np.array([0+2j, 2+2j, 4+2j, 3+4j, 1+4j])  # pentagon

    C_man, bs_man = build_global_tessellation_matrix([cell_A, cell_B, cell_C])

    print()
    print_matrix_info(C_man, bs_man)
    print("\nGlobal tessellation matrix C (real part):")
    print(C_man.real)
    print("\nAuto-detected block sizes:", detect_block_sizes(C_man))

    plot_voronoi_from_matrix(
        C_man, bs_man,
        title="Demo 2: Three hand-crafted Voronoi cells\n"
              "(square + square + pentagon)")

...
class VoronoiGenerator:
    ...
    def get_neighborhood_polygons(self, idx, max_levels):
        ...
        for level in range(1, max_levels + 1):
            ...
            neighbor_points = [self.points[i] for i in neighbor_indices]

            from shapely.geometry import MultiPoint
            multipoint = MultiPoint(neighbor_points)
            hull = multipoint.convex_hull

            if isinstance(hull, Polygon):
                polygons.append((level, hull))
    ...
    def apply_lloyd(self):
        ...
        if self.fixed_point_index is not None:
            ...
            if level_polygons:
                ...
                self.ax.plot(centroids_all.x, centroids_all.y, 'kx', markersize=10)
